// Restore ONLY medications table from backup data
...
async function restoreMedications() {
  try {
    console.log('🔄 Restoring medications from backup...');
    
    // ALL medications data from backup
    const medications = [
      ...
      },
      {
        ...
        name: 'HCG',
        generic_name: 'Human Chorionic Gonadotropin',
        brand_name: 'Pregnyl',
        dosage_form: 'injection',
        strength: '5000 IU',
    ...
    console.log('✅ Successfully restored medications:');
    insertedMeds.forEach((med, index) => {
      console.log(`   ${index + 1}. ${med.brand_name} (${med.strength}) - ${med.name}`);
    });
