...
const weightLossGoals: WeightLossGoal[] = [
  { value: '1-15', label: 'Losing 1-15 lbs' },
  { value: '16-50', label: 'Losing 16-50 lbs' },
  { value: '51+', label: 'Losing 51+ lbs' },
  { value: 'not-sure', label: 'Not sure, I just need to lose weight' }
]

const healthAreas: HealthArea[] = [
  { value: 'better-sex', label: 'Better sex' },
  { value: 'weight-loss', label: 'Weight loss' },
  { value: 'hormone-replacement', label: 'Hormone replacement and TRT' },
  { value: 'skin-health', label: 'Skin health' },
  { value: 'therapy-counseling', label: 'Therapy and counseling' }
]

export interface WeightLossOnboardingEntryProps {
...
export interface PathEntryProps {
...
export function WeightLossOnboardingEntry({
  onGoalSelect,
  onSignInClick,
  progress = 20,
  className
}: WeightLossOnboardingEntryProps) {
  const [selectedGoal, setSelectedGoal] = useState<string>('')

  const handleGoalSelect = (goalValue: string) => {
    setSelectedGoal(goalValue)
    onGoalSelect?.(goalValue)
  }
        ...
        <div className="w-full max-w-md mx-auto relative z-10">
          ...
          <CardHeader className="text-left">
            <CardTitle className="text-2xl bg-gradient-to-b from-gray-900 to-gray-700 dark:from-white dark:to-gray-300 bg-clip-text text-transparent">
              Explore weight loss plans based on your goals, habits, and health history
            </CardTitle>
          </CardHeader>
...
export function PathEntry({
  ...
  const handleAreaToggle = (areaValue: string) => {
    ...
    onAreasSelect?.(newSelectedAreas)
  }
