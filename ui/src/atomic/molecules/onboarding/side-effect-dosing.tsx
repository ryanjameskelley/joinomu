...
export interface DosingOption {
  value: string
  label: string
}

const gastrointestinalDosingOptions: DosingOption[] = [
  { value: 'yes', label: 'Yes' },
  { value: 'no', label: 'No' },
  { value: 'not-sure', label: "I'm not sure" }
]

export interface SideEffectDosingProps {
  onDosingSelect?: (option: string) => void
  onContinue?: () => void
  onSignInClick?: () => void
  progress?: number
  title?: string
...
export function SideEffectDosing({
  onDosingSelect,
  onContinue,
  onSignInClick,
  progress = 96,
  ...
  options = gastrointestinalDosingOptions,
  className
}: SideEffectDosingProps) {
  const [selectedOption, setSelectedOption] = useState<string>('')

  const handleOptionSelect = (optionValue: string) => {
    setSelectedOption(optionValue)
    onDosingSelect?.(optionValue)
  }
