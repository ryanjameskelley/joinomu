...
const challenges: Challenge[] = [
  ...
  { value: 'cost', label: 'Cost of weight loss programs' },
  { value: 'medication-side-effects', label: 'Medication side effects' },
  { value: 'medical-conditions', label: 'Medical conditions that affect my weight loss' },
  { value: 'increased-appetite', label: 'Increased appetite caused by medications' },
  { value: 'other', label: 'Other' }
]

export const programAdherenceReasons: Challenge[] = [
  { value: 'no-results-fast', label: "I wasn't seeing results fast enough" },
  ...
  { value: 'life-obligations', label: 'Stress, travel, family obligations' },
  { value: 'other', label: 'Other' },
  { value: 'usually-consistent', label: "No, I'm usually consistent" }
]

export const programConsistencySupports: Challenge[] = [
...
export interface WeightLossChallengesProps {
  onChallengesSelect?: (challenges: string[]) => void
...
export function WeightLossChallenges({
  onChallengesSelect,
  onTextChange,
  onContinue,
  onSignInClick,
  ...
  maxSelections = 3,
  showTextArea = false,
  textPlaceholder = "Please tell us more...",
  className
}: WeightLossChallengesProps) {
  const [selectedChallenges, setSelectedChallenges] = useState<string[]>([])
  const [textValue, setTextValue] = useState<string>('')

  const handleChallengeToggle = (challengeValue: string) => {
    let newSelectedChallenges: string[]
    
    if (selectedChallenges.includes(challengeValue)) {
      // Remove if already selected
      newSelectedChallenges = selectedChallenges.filter(c => c !== challengeValue)
    } else {
      // Add if not selected, but check max limit
      if (selectedChallenges.length < maxSelections) {
        newSelectedChallenges = [...selectedChallenges, challengeValue]
      } else {
    ...
    onChallengesSelect?.(newSelectedChallenges)
  }
        ...
        <div className="w-full max-w-md mx-auto relative z-10">
          ...
          <CardContent className="flex-1 flex flex-col justify-between">
                ...
                <div className={cn(
                  ...
                  {challengeOptions.map((challenge) => (
                    <button
                      ...
                      disabled={!selectedChallenges.includes(challenge.value) && selectedChallenges.length >= maxSelections}
                      className={cn(
                        ...
                        !selectedChallenges.includes(challenge.value) && selectedChallenges.length >= maxSelections
            ...
            <div className="space-y-4">
              <Button 
                onClick={onContinue}
                disabled={showTextArea ? textValue.trim().length === 0 : selectedChallenges.length === 0}
                className="w-full"
              >
                Continue
              </Button>
