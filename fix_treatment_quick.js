const { createClient } = require('@supabase/supabase-js');
const supabase = createClient('http://127.0.0.1:54321', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImV4cCI6MTk4MzgxMjk5Nn0.EGIM96RAZx35lJzdJsyH-qQwv8Hdp7fsn3W0YpN81IU');

async function fixTreatmentTypes() {
  console.log('Fixing treatment types...');
  const { data, error } = await supabase
    .from('provider_schedules')
    .update({ treatment_types: ['weight_loss'] })
    .eq('provider_id', '1a3803f0-fe96-4649-8e2a-0e06a64c94c1');
  
  if (error) console.error('Error:', error);
  else console.log('Success: Updated schedules');
  
  // Verify
  const { data: check } = await supabase
    .from('provider_schedules')
    .select('*')
    .eq('provider_id', '1a3803f0-fe96-4649-8e2a-0e06a64c94c1')
    .limit(1);
  
  if (check && check.length > 0) {
    console.log('Verification - treatment_types:', check[0].treatment_types);
  }
}

fixTreatmentTypes();
