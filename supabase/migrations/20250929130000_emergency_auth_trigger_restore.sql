-- Emergency fix: Restore the exact auth trigger from the guide
-- Following the safe migration pattern: CREATE OR REPLACE (not DROP + CREATE)
...
-- Attach trigger to auth.users table (safe - will replace if exists)
...
SELECT 
  'Auth system restoration' as status,
  CASE 
