...
BEGIN
    ...
    RAISE LOG 'Auth trigger fired for user: %', NEW.id;
    ...
    RAISE LOG 'Extracted metadata - role: %, name: % %', user_role, first_name, last_name;
    ...
    ELSIF user_role = 'provider' THEN
        ...
        RAISE LOG 'Provider record created with ID: %', new_provider_id;
        ...
        IF new_provider_id IS NOT NULL THEN
            ...
            RAISE LOG 'Provider schedules created for provider: %', new_provider_id;
        END IF;
        
    ELSIF user_role = 'admin' THEN
        INSERT INTO public.admins (profile_id, first_name, last_name)
        VALUES (new_profile_id, first_name, last_name);
        RAISE LOG 'Admin record created for profile: %', new_profile_id;
    END IF;

    RAISE LOG 'Auth trigger completed successfully for user: %', NEW.id;
    RETURN NEW;
EXCEPTION
    WHEN OTHERS THEN
