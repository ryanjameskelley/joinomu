-- Fix the unique constraint issue with provider schedules

-- First check what the unique constraint is
DO $$
DECLARE
    constraint_info RECORD;
BEGIN
    RAISE NOTICE '=== INVESTIGATING UNIQUE CONSTRAINT ISSUE ===';
    
    -- Check constraints on provider_schedules
    FOR constraint_info IN (
        ...
        RAISE NOTICE 'Unique constraint: % - %', constraint_info.conname, constraint_info.definition;
    END LOOP;
END $$;

-- Fix the auth trigger to handle existing schedules gracefully
    ...
    IF user_role = 'provider' AND profile_created THEN
        BEGIN
            ...
            IF existing_schedules = 0 THEN
                -- Create default schedules (Monday-Friday 9-5) with ON CONFLICT handling
                BEGIN
...
DECLARE
    ...
    log_count INTEGER;
BEGIN
    RAISE NOTICE '=== TESTING FIXED AUTH TRIGGER ===';
    ...
    INSERT INTO auth.users (
        ...
        'authenticated',
        'authenticated',
        'fixed.trigger.test@example.com',
        '$2a$10$example.encrypted.password.hash.here',
        NOW(), '', NOW(),
        '{"provider": "email", "providers": ["email"]}',
        '{"role": "provider", "firstName": "Fixed", "lastName": "Test", "specialty": "Cardiology"}',
        NOW(), NOW()
    );
    ...
    RAISE NOTICE 'Fixed trigger results:';
