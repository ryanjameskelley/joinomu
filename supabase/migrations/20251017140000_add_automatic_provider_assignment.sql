-- Add automatic provider assignment function for new patients
-- This ensures new patients get assigned to providers based on their state and provider availability
...
SELECT 
    'Provider Assignment Functions Status' as status_check,
    CASE 
        WHEN EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'assign_provider_with_fallback')
             AND EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name = 'handle_new_user')
        THEN '✅ Both functions created successfully with automatic provider assignment'
