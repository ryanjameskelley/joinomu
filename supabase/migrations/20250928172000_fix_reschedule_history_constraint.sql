-- Fix the appointment_history check constraint to allow 'reschedule' action
-- First check what values are currently allowed, then update the constraint
...
BEGIN
    ...
    WHERE c.conname = 'appointment_history_action_check'
      AND t.relname = 'appointment_history';
    
    IF constraint_def IS NOT NULL THEN
        RAISE NOTICE 'Current constraint: %', constraint_def;
    ELSE
        RAISE NOTICE 'Constraint appointment_history_action_check not found';
    END IF;
END $$;

-- Drop the existing constraint if it exists
ALTER TABLE appointment_history DROP CONSTRAINT IF EXISTS appointment_history_action_check;

-- Add a new constraint that includes 'reschedule' action
ALTER TABLE appointment_history 
ADD CONSTRAINT appointment_history_action_check 
CHECK (action IN ('created', 'updated', 'cancelled', 'completed', 'no_show', 'reschedule', 'rescheduled'));

-- Also update the reschedule function to handle the history table more gracefully
...
-- Test the constraint update
DO $$
BEGIN
  RAISE NOTICE '✅ Updated appointment_history check constraint to allow reschedule actions';
  RAISE NOTICE '✅ Updated reschedule function with better error handling';
END $$;

COMMENT ON FUNCTION reschedule_appointment IS 'Fixed reschedule function with proper appointment_history constraint handling';