-- Enhanced auth trigger that supports current database structure and automatic provider schedules
...
DECLARE
  ...
  day_name TEXT;
BEGIN
  -- Extract user metadata safely from both possible locations
  user_role := COALESCE(
  ...
  -- Create role-specific records based on current table structure
  ...
  ELSIF user_role = 'provider' THEN
    ...
    -- Create provider schedule automatically (Monday-Friday, 9-5)
